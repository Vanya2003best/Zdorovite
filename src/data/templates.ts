...
/** All currently-shipped templates. Used for runtime validation when reading
 *  customization from DB or accepting it from form input. */
export const TEMPLATE_NAMES: readonly TemplateName[] = [
  "premium", "cozy", "luxury", "studio", "cinematic", "signature",
] as const;

/** Coerce a possibly-stale template string (DB rows from before we dropped
 *  Minimal + Sport in 2026-04 still may have those values) to a valid one.
 *  Anything we don't recognise falls back to "premium" — a free, neutral
 *  default that won't paywall a returning trainer. */
export function normalizeTemplate(t: string | undefined | null): TemplateName {
  return (TEMPLATE_NAMES as readonly string[]).includes(t ?? "")
    ? (t as TemplateName)
    : "premium";
}
...
export const templates: Record<TemplateName, TemplateStyles> = {
  ...
  cozy: {
    ...
    coverHeight: "h-[150px] sm:h-[200px]",
    coverOverlay: "",
    avatarStyle: "w-[120px] h-[120px] sm:w-[140px] sm:h-[140px] rounded-full border-4 border-[#fdf6ec] bg-[#fbbf77] shadow-[0_8px_24px_rgba(164,95,30,0.22)]",
    nameStyle: "text-[36px] sm:text-[44px] font-semibold tracking-tight text-[#2d2418] leading-[1.05]",
    tagStyle: "text-[15px] sm:text-[17px] text-[#8a7559] leading-relaxed",
    metaStyle: "text-[14px] text-[#6b5a41]",
    sectionBorder: "",
    sectionTitleStyle: "text-[12px] uppercase tracking-[0.12em] text-orange-600 font-semibold mb-3",
    sectionPadding: "px-6 py-7 sm:py-8",
    bodyText: "text-[15px] text-[#44372b] leading-[1.65]",
    // Services — rounded white cards (larger for desktop public render)
    svcContainerStyle: "grid gap-2.5",
    svcItemStyle: "bg-white rounded-2xl p-4 sm:p-5 flex justify-between items-center shadow-[0_2px_10px_rgba(164,95,30,0.07)] gap-4",
    svcNameStyle: "text-[15px] sm:text-[16px] font-medium text-[#2d2418]",
    svcDescStyle: "text-[13px] text-[#8a7559] mt-1",
    svcPriceStyle: "bg-[#fef3e0] text-orange-700 px-3.5 py-2 rounded-full text-[13px] font-semibold whitespace-nowrap",
    // Packages — container-query breakpoints so the grid responds to the
    // actual content column width (the surrounding @container in
    // TemplateProfile sets the query context). Avoids letter-by-letter wrap
    // on package names when the canvas / viewport is narrow.
    pkgContainerStyle: "grid grid-cols-1 @[520px]:grid-cols-2 @[800px]:grid-cols-3 gap-4",
    pkgCardStyle: "bg-white rounded-2xl p-5 sm:p-6 shadow-[0_4px_16px_rgba(164,95,30,0.08)] relative",
    pkgFeaturedStyle: "bg-gradient-to-br from-[#fef3e0] to-[#fbbf77] rounded-2xl p-5 sm:p-6 shadow-[0_8px_24px_rgba(249,115,22,0.28)] relative text-[#2d2418]",
    pkgFeaturedBadge: "absolute -top-2.5 left-4 bg-orange-600 text-white text-[11px] px-3 py-1 rounded-full font-semibold uppercase tracking-[0.05em] shadow-sm",
    pkgNameStyle: "text-[12px] uppercase tracking-[0.08em] text-[#8a7559] font-semibold",
    pkgPriceStyle: "text-[28px] font-semibold tracking-tight text-[#2d2418] leading-none mt-2",
    pkgItemStyle: "text-[13px] text-[#44372b] leading-relaxed",
    pkgItemPrefix: "",
    pkgButtonStyle: "mt-5 w-full py-3 px-4 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-full text-[14px] font-semibold shadow-[0_6px_16px_rgba(234,88,12,0.28)] hover:opacity-90 transition",
    // Reviews — rounded white cards
    revCardStyle: "bg-white rounded-2xl p-5 sm:p-6 mb-3 shadow-[0_2px_10px_rgba(164,95,30,0.06)]",
    revNameStyle: "text-[16px] sm:text-[17px] font-semibold text-[#2d2418]",
    revDateStyle: "text-[13px] text-[#a08668]",
    revTextStyle: "text-[15px] sm:text-[16px] text-[#44372b] leading-relaxed",
    revStarsStyle: "text-[14px] text-orange-600 tracking-wider",
    // CTA bar
    ...
    certTextStyle: "text-[15px] text-[#44372b]",
  },
};
