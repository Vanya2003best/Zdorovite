    ...
    <div className="min-h-screen bg-[radial-gradient(800px_400px_at_10%_-10%,rgba(16,185,129,0.08),transparent_60%),radial-gradient(600px_400px_at_100%_0%,rgba(20,184,166,0.06),transparent_60%),linear-gradient(180deg,#f8fafc_0%,#ffffff_35%)]">
          ...
          <div className="sm:grid sm:grid-cols-[180px_1fr_220px] sm:gap-7 sm:items-center">
            ...
            <div className="flex sm:block gap-3.5">
              ...
              <div className="sm:hidden">
                ...
                <p className="text-[13px] text-slate-600 leading-snug">{trainer.tagline.split("—")[0].trim()}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <p className="hidden sm:block text-base text-slate-600 leading-relaxed mt-1.5 max-w-[580px]">{trainer.tagline}</p>
            ...
            <div className="hidden sm:flex flex-col gap-2.5 min-w-[220px]">
              ...
              <button className="w-full h-14 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl text-base font-medium shadow-[0_10px_30px_rgba(16,185,129,0.18)] hover:brightness-105 transition">
                Zarezerwuj sesję
              </button>
              <button className="w-full flex items-center justify-center gap-2.5 py-3 border border-slate-200 rounded-xl text-sm font-medium text-slate-900 bg-white hover:border-slate-400 transition">
      ...
      <div className="mx-auto max-w-[1200px] px-3.5 sm:px-6 sm:grid sm:grid-cols-[1fr_340px] sm:gap-10 py-8 sm:py-10">
        <div>
          ...
          <section className="mb-12">
            ...
            <div className="bg-white/75 backdrop-blur-lg border border-white/70 rounded-[20px] p-5 sm:p-7 shadow-sm">
              <p className="text-[15px] text-slate-700 leading-[1.65]">{trainer.about}</p>
          ...
          <section className="mb-12">
            ...
            <div className="grid sm:grid-cols-2 gap-3.5">
              {trainer.services.map((svc) => (
                <div key={svc.name} className="bg-white/80 backdrop-blur-sm border border-white/70 rounded-[18px] p-5 sm:p-5.5 shadow-sm flex flex-col gap-2.5">
        ...
        <aside className="hidden sm:block">
          <div className="sticky top-[140px] bg-white/88 backdrop-blur-xl backdrop-saturate-[1.4] border border-white/70 rounded-[20px] shadow-[0_24px_48px_-20px_rgba(2,6,23,0.12)] p-6">
            <h3 className="text-[15px] font-semibold tracking-tight mb-3.5">Zarezerwuj termin</h3>
            {/* Date picker */}
            <div className="flex items-center justify-between mb-3.5 p-3 bg-slate-50 rounded-xl">
              <button className="w-7 h-7 rounded-lg bg-white border border-slate-200 inline-flex items-center justify-center">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M15 18l-6-6 6-6" /></svg>
              </button>
              <div className="text-center">
                <div className="text-[13px] font-semibold">Czwartek · 23 kwietnia</div>
                <div className="text-[11px] text-slate-500">8 dostępnych terminów</div>
              </div>
              <button className="w-7 h-7 rounded-lg bg-white border border-slate-200 inline-flex items-center justify-center">
            ...
            <div className="grid grid-cols-4 gap-1.5 mb-4">
              {["08:00", "09:00", "10:00", "11:00", "15:00", "16:00", "17:00", "18:00"].map((time, i) => (
                <div key={time} className={`py-2 text-center border rounded-[9px] text-xs cursor-pointer transition ${i === 4 ? "bg-slate-900 text-white border-slate-900" : i === 0 || i === 3 ? "text-slate-400 bg-slate-50 border-slate-200 cursor-not-allowed" : "bg-white border-slate-200 hover:border-slate-400"}`}>
                  {time}
                </div>
              ))}
            </div>
            <div className="text-[13px] text-slate-600 mb-2.5">
              Wybrana usługa: <strong className="text-slate-900">Rehabilitacja pourazowa</strong>
            </div>
            <div className="flex justify-between items-baseline pt-3.5 border-t border-slate-200 mt-3.5">
              <span className="text-[13px] text-slate-500">Do zapłaty</span>
              <span className="text-[22px] font-semibold tracking-tight">180 zł</span>
            </div>
            <button className="w-full mt-3.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl py-3.5 text-[15px] font-medium shadow-[0_10px_30px_rgba(16,185,129,0.18)] hover:brightness-105 transition">
            ...
            {/* Message button */}
            <button className="w-full flex items-center justify-center gap-2.5 py-3 mt-2 border border-slate-200 rounded-xl text-sm font-medium text-slate-900 bg-white hover:border-slate-400 transition">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" /></svg>
              Napisz wiadomość
            </button>
          </div>
    ...
    <div className={`min-h-screen ${s.pageBg}`}>
      <div className="mx-auto max-w-2xl">
        <div className={`${s.coverBg} ${s.coverHeight}`}>
          {isSport && (
            <>
              <img src="https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=800&h=400&fit=crop" alt="" className="w-full h-full object-cover opacity-50" />
        ...
        <div className={`${isSport ? "px-6 pb-5" : "px-6"} -mt-9 relative z-10`}>
          ...
          <p className={`mt-0.5 ${s.tagStyle}`}>
            {isSport ? trainer.specializations.map((sp) => getSpecLabel(sp)).join(" · ") : trainer.tagline}
          </p>
...
export default async function TrainerProfilePage(props: PageProps<"/trainers/[id]">) {
  ...
  if (!found) notFound();
  const trainer = found;
  if (trainer.customization.template === "premium") return <PremiumProfile trainer={trainer} />;
