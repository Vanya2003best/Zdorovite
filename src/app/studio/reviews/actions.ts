...
export async function setReviewReply(
  ...
  try {
    if (typeof reviewId !== "string" || reviewId.trim().length === 0) {
      return { error: "Brak id opinii." };
    }
    if (typeof text !== "string") return { error: "Nieprawidłowa odpowiedź." };
    ...
    console.error("setReviewReply failed", err);
...
export async function togglePinReview(reviewId: string): Promise<ActionResult> {
  try {
    ...
    console.error("togglePinReview failed", err);
    return { error: DEFAULT_ERROR };
  }
}
