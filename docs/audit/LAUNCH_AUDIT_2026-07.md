...
> **СТАТУС 2026-07-06 (волны 1–2, коммиты 99c4404 · 619493f · 7c724ec · 1bf328e · 58c9ebd):
> ЗАКРЫТО ВСЁ из P1/P2 и вся P3-пачка**, кроме отмеченного ниже. Детали — в сообщениях
> коммитов. Поправки к реестру по фактам исполнения:
> - `nav-items.tsx` — НЕ dead code (импортирует живой StudioNavMenu из EditorClient) —
>   оставлен, вычищен от kupony; остальной dead code удалён (+StudioPageTitle.tsx).
> - Отзывы: правило «одна опиния на тренера» (UNIQUE 001) сохранено осознанно;
>   RLS-политика restrictive «insert completed booking only» проверена живыми ролями.
> - Осталось открытым: деталь «+5 PLN credit» у счётчика опиний (фейк-обещание,
>   решить: убрать или сделать реальным); «Confirm email» в консоли Supabase — Ваня;
>   миграция 024 на проде — проверить перед включением Finanse.
